          ...
          >
            ...
            <div className="p-4 sm:p-6 md:p-8 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
                <ShoppingBag size={16} strokeWidth={1.5} className="text-accent flex-shrink-0 sm:size-5 md:size-5" />
                <Typography variant="h4" className="text-xs sm:text-sm md:text-base font-bold text-foreground truncate">Tu Bolsa</Typography>
                <span className="text-[8px] sm:text-[9px] bg-muted text-foreground/60 px-1.5 sm:px-2 py-0.5 rounded-full font-bold flex-shrink-0">
                  {cartItems.length}
                </span>
              </div>
              <button onClick={() => setIsCartOpen(false)} className="hover:text-accent transition-colors flex-shrink-0">
                ...
                vendors.map((vendor) => (
                  <div key={vendor} className="space-y-4 sm:space-y-5 md:space-y-6">
                    <div className="flex items-center gap-1.5 sm:gap-2 border-b border-border pb-2">
                       <Store size={12} className="text-accent flex-shrink-0 sm:size-3.5 md:size-3.5" />
                    ...
                    <div className="space-y-6 sm:space-y-7 md:space-y-8">
                      {groupedItems[vendor].map((item) => (
                        <div key={`${item.id}-${item.size}`} className="flex gap-3 sm:gap-4 md:gap-6 group">
                          <div className="relative w-14 sm:w-16 md:w-20 aspect-[3/4] bg-muted rounded-lg sm:rounded-xl overflow-hidden border border-border flex-shrink-0">
                            ...
                            <div className="flex justify-between items-start gap-1">
                              ...
                              <button 
                                ...
                                className="text-foreground/30 hover:text-accent transition-colors flex-shrink-0"
                              >
                            ...
                            <div className="flex justify-between items-end gap-2">
                              ...
                              <Typography variant="body" className="text-[8px] sm:text-xs md:text-xs font-bold text-foreground flex-shrink-0">
